  ...
  this.handleMessage = function(request, sender, send_response) {
    switch(request.type) {
      case 'REQ_SCRIPT_LIST':
        ...
        return true;
      case 'INJECTED':
        if (_handler.port === undefined) break;
        _handler.port.postMessage(request);
        break;
      case 'SCRIPT_SELECTED':
        _handler.selectedScript[request.tab] = request.data;
        break;
      case 'REQ_SELECTED_SCRIPT':
        if (Object.keys(_handler.selectedScript) === 0)
          break;
      ...
      case 'SCRIPT_ADDED':
        ...
        if (data.name && data.url) {
          // we don't care about name conflict :)
          if (_scripts.every(hasUrl(data.url))) {
            _scripts.push(data);
            storeScriptData(_scripts);
          } else
            send_response({type:request.type});
        }
        break;
      case 'SCRIPT_DELETED':
        break;
...
chrome.runtime.onInstalled.addListener(function(detail) {
  ...
  switch(detail.reason) {
    case 'install':
      ...
      storeScriptData(defaultScripts);
      break;
    case 'update':
