  ...
  this.handleMessage = function(request, sender, send_response) {
    switch(request.type) {
      ...
      case 'INJECTED':
        ...
        break;
      case 'INJECT_LIST':
        var selected = _handler.selectedScript; 
        if (!selected[request.tabID]) selected[request.tabID] = {};
        selected[request.tabID][request.tabURL] = request.data;
        break;
      case 'REQ_SELECTED_LIST':
        var script = _handler.selectedScript[sender.tab.id];
        if (Object.keys(_handler.selectedScript).length === 0 ||
           !script ||
           !script[sender.tab.url]) {
          send_response({type:request.type, listArray:[]});
          break;
        }
        send_response({type:request.type, listArray:script[sender.tab.url]});
        break;
      case 'ADD_LIST':
      ...
      case 'REMOVE_LIST':
        ...
        if (_scripts.every(hasScript(request.uid))) {
          _script = _scripts.filter(hasScript(request.uid));
          storeScriptData(_scripts);
          send_response({type:request.type, status:'complete'});
        }
        break;
