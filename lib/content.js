...
function inject(scriptInfo) {
  ...
  tag.setAttribute('data-soak', 'main');
...
function onLoad() {
  chrome.extension.sendMessage({type:'REQ_SELECTED_LIST'}, function(response) {
    if (response.listArray.length) {
      console.log('selected : ' + response.listArray.length);
      for(var i = 0 ; i < response.listArray.length ; i ++) {
        inject(response.listArray[i]);
      }
    }
  });
  addConsoleInterface();
